...
CScript _createmultisig(const Array& params)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        CKonjungatecoinAddress address(ks);
...
Value createmultisig(const Array& params, bool fHelp)
{
    ...
    CKonjungatecoinAddress address(innerID);
...
Value getnewaddress(const Array& params, bool fHelp)
{
    ...
    return CKonjungatecoinAddress(keyID).ToString();
}


CKonjungatecoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CKonjungatecoinAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the current Konjungate address for receiving payments to this account.\n"
            "\nArguments:\n"
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    CKonjungatecoinAddress address(params[0].get_str());
    if (!address.IsValid())
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CKonjungatecoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CKonjungatecoinAddress& address = item.first;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress \"Konjungateaddress\" amount ( \"comment\" \"comment-to\" )\n"
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CKonjungatecoinAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CKonjungatecoinAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CKonjungatecoinAddress(address).Get())->second);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    ...
    CKonjungatecoinAddress addr(strAddress);
    if (!addr.IsValid())
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    ...
    // Bitcoin address
    CKonjungatecoinAddress address = CKonjungatecoinAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Konjungate address");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            ...
            "\nSent an amount from an account to a Konjungate address.\n"
    ...
    string strAccount = AccountFromValue(params[0]);
    CKonjungatecoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Konjungate address");
    CAmount nAmount = AmountFromValue(params[2]);
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The Konjungate address is the key, the numeric amount in IC is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    set<CKonjungatecoinAddress> setAddress;
    vector<pair<CScript, int64_t> > vecSend;

    int64_t totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CKonjungatecoinAddress address(s.name_);
        if (!address.IsValid())
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    ...
    return CKonjungatecoinAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CKonjungatecoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CKonjungatecoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CKonjungatecoinAddress, tallyitem>::iterator it = mapTally.find(address);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; Konjungate server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new Konjungate stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid Konjungate stealth address."));
        return result;
    };
